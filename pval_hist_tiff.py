...
pdf_filename = os.path.join(results_dir, 'fig2-pval-hist.pdf')
tiff_filename = os.path.join(results_dir, 'fig2-pval-hist.tiff')
plt.savefig(pdf_filename)
plt.savefig(tiff_filename, dpi=600, pil_kwargs={"compression": "tiff_lzw"})
plt.close()

logger.info(f'{filename} -> {pdf_filename} {tiff_filename}')
